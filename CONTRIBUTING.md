...
4. **No production implementation** until Foundation approval (G1), Build Ladder authorization (G2), and explicit per-rung authorization (G3). All three are required; the first two do not imply the third. See `docs/05-governance/gate-model.md`.
...
When a specific rung is authorized (gate G3, fixture-only):
...
Follow `docs/05-governance/git-policy.md`. Do not force-push protected branches. Do not commit lockfiles until a rung is authorized at gate G3.
